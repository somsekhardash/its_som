...
const useHttp = (): [boolean, any, any, (url: string, body: any) => void] => {
  ...
  const makeCall = (url: string, body: any) => {
    body.method == "PUT" && window.localStorage.setItem(url, body.body);
    fetch(url, { ...body })
  ...
  const makeTheCall = (url: string, body: any) => {
    setIsLoading(true);
    if (body.method == "PUT") makeCall(url, body);
    else localStorage[url] ? getFromLocalHost(url) : makeCall(url, body);
  };
